...
**/
								...
								if (*RootPtr == NULL)
								{
																// The BT is empty so we can add first list
																if ((tmp = malloc(sizeof *tmp)) == NULL)
																{
																								// Allocation on heap failed
																								return;
																}
																tmp->Cont = Content;
																tmp->LPtr = NULL;
...
**/
								...
								Leftmost_Postorder(tmp, &p_stack, &b_stack);

								// Until stack of pointers is empty
								while (!(SEmptyP(&p_stack)))
								{
																tmp = STopPopP(&p_stack);
																SPushP(&p_stack, tmp);
																...
																if (STopPopB(&b_stack))
																{
																								...
																								Leftmost_Postorder(tmp->RPtr, &p_stack, &b_stack);
																}
																else
																{
																								// Coming back -- print
																								BTWorkOut(tmp);
																								// Pop from stack of pointers
																								STopPopP(&p_stack);
																}
								}
}
