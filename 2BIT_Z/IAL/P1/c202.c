...
*/
    /// Does pointer points to NULL (nowhere) ? Dereferencing would lead to UB
		if (s == NULL)
		{
		...
		{
			/// Set top as -1, means stack is empty
	  	s->top = -1;
		}
}
...
*/
    ...
    /// Value -1 means that stack is empty, compare them and return value
	  return (s->top == -1);
}
...
*/
		...
		/// Max index is one less then max value
...
*/
    ...
    /// Is stack empty ? Print error
	  if (stackEmpty(s))
		...
		{
    	/// Save character from top of the stack to pointer-to-char
			*c = s->arr[s->top];
		}
}
...
*/
    ...
    /// Is stack empty ? Dont do anything
	  if (stackEmpty(s))
		{
			return;
		}
		...
		{
			/// Decrease poitner to top of stack by one
			s->top--;
		}
}
...
*/
    ...
    /// Trying to push to full stack is an error
	  if (stackFull(s))
		{
		...
		{
			/// Increase poitner to top element in stackError
			s->top++;
			/// Save character on that position
			s->arr[s->top] = c;
		}
}
