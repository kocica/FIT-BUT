...
void untilLeftPar ( tStack* s, char* postExpr, unsigned* postLen ) {
  ...
  /// Read characters from top of stack until its empty or left parentheses '('
  /// and save these characters to array onto positions postlen and more
  while (!stackEmpty(s))
  {
    stackTop(s, &c);
    stackPop(s);
...
void doOperation ( tStack* s, char c, char* postExpr, unsigned* postLen ) {
  ...
  /// If stack is not empty, get top character
  if (!stackEmpty(s))
  {
    stackTop(s, &new_c);
  }

  if (stackEmpty(s) || new_c == '(' || (new_c == '+' && c != '-' && c != '+') ||
      (new_c == '-' && c != '-' && c != '+'))
  {
    stackPush(s, c);
  }
...
char* infix2postfix (const char* infExpr) {
  char tempChar;
  /// Pointer to character (array of character if terminated w/ null terminating character)
  /// Allocation enough memory on heap and returning NULL if allocation failed
  ...
  /// Stack instantion & init stack
  tStack stack;
  stackInit(&stack);

  /// Index to postfix expression array
  unsigned int postLen = 0;

  /// Get length of infix expression
  size_t str_len = 0;
  for (int i = 0; infExpr[i]; i++)
  {
    str_len++;
  }
  
  /// Iteration through infix expression array
  for (size_t i = 0; i < str_len; i++)
  {
    ...
    if ((tempChar >= '0' && tempChar <= '9') ||
        ...
        (tempChar >= 'A' && tempChar <= 'Z'))
    {
      /// Add to postfix array
      postExpr[postLen++] = tempChar;
    }
    /// Is it left parentheses ?
    else if (tempChar == '(')
    {
      /// Push it to the top of stack
      stackPush(&stack, tempChar);
    }
    /// Is it right parentheses ?
    else if (tempChar == ')')
    {
      untilLeftPar(&stack, postExpr, &postLen);
    }
    /// Is it equal operator
    else if (tempChar == '=')
    {
      char c;
    ...
    {
      doOperation(&stack, tempChar, postExpr, &postLen);
    }
  }
  return postExpr;
}
