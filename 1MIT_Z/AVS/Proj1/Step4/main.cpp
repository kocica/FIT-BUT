...
int main(int argc, char* argv[])
{
  ...
  // Transponse weights for easies memory access
  transpose2D( weight1, imagePixels, layerSize );
  transpose2D( weight2, imagePixels, layerSize );
  transpose2D( weight3, imagePixels, layerSize );
...
)
{
  ...
  for (size_t i = 0; i < neuronCount; i++)
  {
    ...
    {
      result += input[ idx ] * weight[ ( i * inputSize ) + idx ];
    }
  
    output[i] = max( 0.0f, result + bias[i] );
  }
}
