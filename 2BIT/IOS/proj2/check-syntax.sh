...
A_MAX=$(cat "${outf}" | grep started | sort -rnk4 | grep -m1 A | sed "s#^[0-9]*[^0-9]*\([0-9]*\).*#\1#g")
C_MAX=$(cat "${outf}" | grep started | sort -rnk4 | grep -m1 C | sed "s#^[0-9]*[^0-9]*\([0-9]*\).*#\1#g")
...
}
{
	...
	else if ( akce == "trying" && $7 == "to" && $8 == "leave" )
	{
		Trying[id]++
