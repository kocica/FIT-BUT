...
class CTests
{
	...
	public static function runTest($dir, $filename)
	{
		...
		$temp = tmpfile();
		exec('php5.6 ' . CArguments::$parse_scrpt . ' < ' . $dir . "/" . $filename . 'src > ' . stream_get_meta_data($temp)['uri'], $wth, $rc);
		...
		if ($rc !== 0)
		{
			if ($testRC !== (string)$rc)
			{
				echo "<font size=\"2\" color=\"red\">FAIL: Return codes of parse.php and .rc file doesnt match</font><br>";
				$testFailed++;
				return;
			}
		}
		...
		$temp2 = tmpfile();
		exec('python3.6 ' . CArguments::$int_scrpt . ' --source=' . stream_get_meta_data($temp)['uri'] . ' > ' . stream_get_meta_data($temp2)['uri'], $wth, $rc);

		fseek($temp2, 0);

		if ($testRC !== (string)$rc)
		{
			echo "<font size=\"2\" color=\"red\">FAIL: Return codes of interpret.py and .rc file doesnt match</font><br>";
			$testFailed++;
			return;
		}
		...
		if ($rc === 0)
		{
			exec('diff ' . stream_get_meta_data($temp2)['uri'] . ' ' . $dir . "/" . $filename . 'out  > ' . stream_get_meta_data($temp3)['uri'], $wth, $rc);
			fseek($temp3, 0);
			if (fread($temp3, 1024) !== "")
			{
