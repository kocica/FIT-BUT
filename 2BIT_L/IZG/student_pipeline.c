...
VertexIndex gpu_computeGLVertexID(
  ...
  /// Indexing je aktivní, pokud buffer indexů není NULL.
	...
	/// Indices is NULL -- indexing is not active
	if (indices == NULL)
		return vertexShaderInvocation;
	/// Else -- indexing is active
	else
...
void const *gpu_computeVertexAttributeDataPointer(
  ...
  uint64_t offset = head->offset + (head->stride * gl_VertexID);
  void const * vertexAttributeDataPointer = (void const *)((uint64_t)head->buffer + offset);

 	return vertexAttributeDataPointer;
}
...
void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
  ...
  size_t i;
	...
	id = gpu_computeGLVertexID(puller->indices, vertexShaderInvocation);

	for (i = 0; i < MAX_ATTRIBUTES; i++)
		output->attributes[i] = ((&puller->heads[i])->enabled == 0
			? NULL
			: gpu_computeVertexAttributeDataPointer(&puller->heads[i], id));
}
