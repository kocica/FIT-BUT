...
///  - cpu_setIndexing()
    ...
    /// Crete buffers & fill with bunny vertices/indices data
    ...
    cpu_setVertexPullerHead(phong.gpu, phong.puller, 0, phong.vbo,
      floatSize*0, floatSize*6);
    cpu_setVertexPullerHead(phong.gpu, phong.puller, 1, phong.vbo,
      floatSize*3, floatSize*6);

    cpu_enableVertexPullerHead(phong.gpu, phong.puller, 0);
    cpu_enableVertexPullerHead(phong.gpu, phong.puller, 1);

    cpu_setIndexing(phong.gpu, phong.puller, phong.ebo, floatSize);
}
...
///  - getUniformLocation()
  	...
  	const int verticiesCount = 2092 * 3;

    cpu_useProgram(phong.gpu, phong.programID);

    cpu_bindVertexPuller(phong.gpu, phong.puller);

    UniformLocation viewMatrixUniform = getUniformLocation(phong.gpu,
      "viewMatrix");

    cpu_uniformMatrix4fv(phong.gpu, viewMatrixUniform, (float *) &viewMatrix);

    UniformLocation projectionMatrixUniform = getUniformLocation(phong.gpu,
      "projectionMatrix");

    cpu_uniformMatrix4fv(phong.gpu, projectionMatrixUniform,
      (float *) &projectionMatrix);

    UniformLocation cameraPositionUniform = getUniformLocation(phong.gpu,
      "cameraPosition");

    cpu_uniform3f(phong.gpu, cameraPositionUniform, cameraPosition.data[0],
      cameraPosition.data[1], cameraPosition.data[2]);

    UniformLocation lightPositionUniform = getUniformLocation(phong.gpu,
      "lightPosition");

    cpu_uniform3f(phong.gpu, lightPositionUniform, phong.lightPosition.data[0],
      phong.lightPosition.data[1], phong.lightPosition.data[2]);

    // Count of bunny verticies
    cpu_drawTriangles(phong.gpu, verticiesCount);
