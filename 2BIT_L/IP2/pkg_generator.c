...
generator()
{
	uint16_t act_len = 0;
	uint16_t i, j, ret = 0;
	uint16_t nb_rx_pkts, port_id, pkt_len;
	uint32_t seqn = 0, ts = 1396306094;
	struct rte_mbuf *pkts[MAX_PKTS_BURST];
	unsigned char pkg[MAX_PKG_LEN];
	//struct ipv4_hdr ip;
	struct ether_hdr eth;
	uint32_t vektor[] = {8, 5, 4, 9, 1, 0, 6, 3, 2, 14, 13, 10, 12, 11, 7};
	...
	while (!quit_signal) {
		for (i = 0; i < rte_eth_dev_count(); i++) {
			...
			for (j = 0; j < nb_rx_pkts; j++) {
				if ((rte_pktmbuf_data_len(pkts[j]) + sizeof(uint16_t) + (2*sizeof(uint32_t))) > MAX_PKG_LEN) {
					// Skip
					continue;
				}
				else if ((act_len + rte_pktmbuf_data_len(pkts[j]) + sizeof(uint16_t) + (2*sizeof(uint32_t))) > MAX_PKG_LEN) {
					...
					//int RN = rand()%MAX_SEKVENCNI_CISLO;

					pkt_len = rte_pktmbuf_data_len(pkts[j]);

					seqn = rte_cpu_to_le_32(seqn);
					ts = rte_cpu_to_le_32(ts);
					pkt_len = rte_cpu_to_le_16(pkt_len);

					rte_memcpy((void *)(pkg + act_len),
							(void *) &seqn, //&vektor[seqn], //&RN,
							sizeof(uint32_t)); seqn++;
					rte_memcpy((void *)(pkg + act_len + sizeof(uint32_t)),
							(void *) &ts,
							sizeof(uint32_t));
					rte_memcpy((void *)(pkg + act_len + (2*sizeof(uint32_t))),
							(void *) &pkt_len,
							sizeof(uint16_t));
					rte_memcpy((void *)(pkg + act_len + sizeof(uint16_t) + (2*sizeof(uint32_t))),
							rte_pktmbuf_mtod(pkts[j], void *),
							pkt_len);

					act_len += pkt_len + (2*sizeof(uint32_t)) + sizeof(uint16_t);
				}
			}
		}
	}

	if (act_len > sizeof(eth)) {
		// Save pkg to pcap file
		rte_memcpy(pkg, &eth, sizeof(eth));
		save_pkg(pkg, act_len);
		act_len = sizeof(eth);
